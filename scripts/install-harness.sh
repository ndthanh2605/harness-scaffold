...
Options:
  ...
  -y, --yes              Accept defaults and skip prompts.
      ...
      --override         On protected-path conflict, back up and replace
                         CLAUDE.md, docs/, and scripts/.
      --force            Overwrite existing files after backing them up.
      --dry-run          Show what would change without writing files.
  -h, --help             Show this help.

Safety:
  If CLAUDE.md, docs/, or scripts/ already exist, interactive installs ask
...
Examples:
  ...
  curl -fsSL https://raw.githubusercontent.com/ndthanh2605/harness-scaffold/main/scripts/install-harness.sh | bash -s -- --yes
  curl -fsSL https://raw.githubusercontent.com/ndthanh2605/harness-scaffold/main/scripts/install-harness.sh | bash -s -- --merge --yes
EOF
}
...
check_protected_target_paths() {
  ...
  [ -e "$TARGET_DIR/CLAUDE.md" ] && conflicts+=("CLAUDE.md")
...

  {
    ...
    printf '  2. Override Back up and replace CLAUDE.md, docs/, and scripts/\n'
...
override_protected_target_paths() {
  ...
  for protected in CLAUDE.md docs scripts; do
...
SOURCE_ROOT=""
SOURCE_MODE="remote"
SOURCE_BASE_URL="${HARNESS_SOURCE_BASE_URL:-https://raw.githubusercontent.com/ndthanh2605/harness-scaffold/main}"
SOURCE_BASE_URL="${SOURCE_BASE_URL%/}"

if [ -n "$SCRIPT_DIR" ] && [ -f "$SCRIPT_DIR/../CLAUDE.md" ] && [ -f "$SCRIPT_DIR/../docs/HARNESS.md" ]; then

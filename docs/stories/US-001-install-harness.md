...
- The installer accepts a specific target path through a command-line option or
  positional argument.
- If `CLAUDE.md`, `docs/`, or `scripts/` already exists in the target, an
  interactive install shows a warning and asks whether to `1. Merge`,
  `2. Override`, or `3. Stop`.
- If `CLAUDE.md`, `docs/`, or `scripts/` already exists in the target, a
...
- Remote install: `curl -fsSL "https://raw.githubusercontent.com/ndthanh2605/harness-scaffold/main/scripts/install-harness.sh?$(date +%s)" | bash -s -- --yes`
- Queries: none.
- API: none.
- Tables: none.
...
targets containing `CLAUDE.md`, `docs/`, or `scripts/` stop with a warning
